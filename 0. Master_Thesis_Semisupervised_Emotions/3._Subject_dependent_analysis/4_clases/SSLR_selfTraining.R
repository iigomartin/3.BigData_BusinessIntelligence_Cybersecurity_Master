...
train <- transform(train, valencearousal = as.factor(valencearousal))
cls <- which(colnames(train) == "valencearousal")
labeled.index <- createDataPartition(train$valencearousal, p = .2, list = FALSE)
train[-labeled.index,cls] <- NA
cl <- makePSOCKcluster(10)
registerDoParallel(cl)
rf <- rand_forest(trees = 100, mode = "classification") %>%
  set_engine("randomForest")
m <- selfTraining(learner = rf,max.iter=10) %>% fit(valencearousal ~ ., data = train)
stopCluster(cl)
predictions=predict(m,test)%>%bind_cols(test)
data=predictions$.pred_class
data <- factor(data, levels = c('1', '2','3','4'))
reference = factor(predictions$valencearousal)
reference <- factor(reference, levels = c('1', '2','3','4'))
